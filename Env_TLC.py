...
"""
    def _init_(self, programID, tlsID) :
        self.id = tlsID
    ...
    def traffic_volume (self, ):
        ...
        IBVolume = [traci.lane.getLastStepVehicleNumber(laneID) for laneID in IBlaneList]
        OBVolume = [traci.lane.getLastStepVehicleNumber(laneID) for laneID in OBlaneList]
        
        IBWaitingTime = [traci.lane.getWaitingTime(laneID) for laneID in IBlaneList]
        IBOccupancy = [traci.lane.getLastStepOccupancy(laneID) for laneID in IBlaneList]
        
        observation = np.row_stack((IBVolume,OBVolume,IBWaitingTime,IBOccupancy))